...
char* OBJDUMP_ARR[] = {"armv5l-objdump", "armv5l-objdump", "m68k-objdump", "mips-objdump", "mipsel-objdump", "powerpc-objdump", "sh4-objdump", "sparc-objdump", "objdump", "objdump"};
char* ARCHS_ARR[] = { "arm" , "arm5n", "m68k", "mips", "mpsl", "ppc", "sh4", "spc", "x64", "x86"};
...
char* execute_objdump(char* objdump, char* file, char* addr){

  FILE *fp;
  char path[1024];
  uint cmd_size = snprintf(NULL, 0, "%s -d %s | awk -v RS= '/^[0]*%s+ <.*>/' | sed 1d | awk 'BEGIN{FS=\"\t\"}{ print $2}' | tr -d '\\040\\011\\012\\015'", objdump, file, addr);
  char cmd[cmd_size+2];
  snprintf(cmd, cmd_size+1, "%s -d %s | awk -v RS= '/^[0]*%s+ <.*>/' | sed 1d | awk 'BEGIN{FS=\"\t\"}{ print $2}' | tr -d '\\040\\011\\012\\015'", objdump, file, addr);
  cmd[cmd_size+1] = 0;
  ...
  while (fgets(path, sizeof(path)-1, fp) != NULL) {
  	if(result)
  		size = strlen(result);
    result = realloc(result, size + strlen(path)+1);
    result[0] =0;
    strcat(result, path);
  }
  pclose(fp);
...
void get_addr(char* addr, char* addr1){
	if (!addr1 || strlen(addr1) < 8){
		addr =NULL;
		return;
	}
	int i = 8;
	int j = 0;
	while( i>=0){
		addr[i] = addr1[strlen(addr1)-j];
		j++;
		i--;
	}
}

void get_objdump(char* objdump, char* path){
	if(!path){
		objdump = NULL;
		return;
	}
	...
	var[k] = 0;
	int i =0;
	for (i= 0; i< OBJDUMP_SIZE; i++)
		if (!strcmp(ARCHS_ARR[i],var))
			break;
	if (i==OBJDUMP_SIZE){
		objdump=NULL;
		return;
	}
	strcpy(objdump, OBJDUMP_ARR[i]);
}
...
void evaluate(char* db_path, char* DB){
	...
	while(fgets ( line, sizeof line, fp_read ) != NULL ){
		...
		char objdump[30];
		get_addr(addr, addr1); 
		get_objdump(objdump, full_path1 );
		if(!addr ||  !objdump)
			continue;
		char* asm1 = execute_objdump(objdump, full_path1, addr);

		if (!asm1)
			continue;

		get_addr(addr, addr2);
		get_objdump(objdump, full_path2 );
		if(!addr ||  !objdump)
			continue;
		char* asm2 = execute_objdump(objdump, full_path2, addr);
		if (!asm2)
			continue;
